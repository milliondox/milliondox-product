...
public function fetchTasksByDate(Request $request)
{
    $selectedDate = $request->input('date');

    // Fetch tasks based on the selected date and user
    $tasks = Tasks::where('status', '!=', 'deleted')
                ->where('user_id', auth()->user()->id)
                ->whereDate('taskDeadline', $selectedDate) // Assuming `task_date` is the field in the DB
                ->get();
