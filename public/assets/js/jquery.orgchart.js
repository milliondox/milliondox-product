...
(function($) {
    ...
    $.fn.orgChart.defaults = {
        data: [{id:1, name:'Root', description:'', parent: 0}],
        showControls: false,
        allowEdit: false,
    ...
    function OrgChart($container, opts){
        ...
        this.startEdit = function(id, isDescription) {
            var inputElement;
            if (isDescription) {
                inputElement = $('<textarea class="org-input">' + nodes[id].data.description + '</textarea>');
            } else {
                inputElement = $('<input class="org-input" type="text" value="' + nodes[id].data.name + '"/>');
            }
            ...
            var commitChange = function() {
                if (isDescription) {
                    nodes[id].data.description = inputElement.val();
                } else {
                    nodes[id].data.name = inputElement.val();
                }
                ...
                var replacedElement;
                if (isDescription) {
                    replacedElement = $('<p>' + nodes[id].data.description + '</p>');
                } else {
                    replacedElement = $('<h2>' + nodes[id].data.name + '</h2>');
                }

                if (opts.allowEdit && !isDescription) {
                    replacedElement.click(function() {
                        self.startEdit(id, isDescription);
                    });
                }

                inputElement.replaceWith(replacedElement);
            };
        ...
        this.newNode = function(parentId, name, description) {
            var nextId = Object.keys(nodes).length;
            while (nextId in nodes) {
                nextId++;
            }
            ...
            self.addNode({
                id: nextId,
                name: name || '', // If name is not provided, set it to empty string
                description: description || '', // If description is not provided, set it to empty string
                parent: parentId
            });
        }
    ...
    function Node(data){
        ...
        this.formatNode = function(opts){
            var nameString = '<h2>'+self.data.name+'</h2>';
            var descString = '';

            if(opts.showDescription && typeof data.description !== 'undefined'){
                descString = '<p>'+self.data.description+'</p>';
            }
