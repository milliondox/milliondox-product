  ...
  $(window).resize(function () {
    ...
    if (widthwindaw <= 1184) {
      ...
      if ($(window).width() <= 1184) {
        $(document).on("click", function(event) {
          if (!$(event.target).closest('.logo-wrapper').length && !$(event.target).closest('.sidebar-wrapper').length) {
      ...
      $(".toggle-sidebar").on("click", function(event) {
        event.stopPropagation();
        $(this).children('.sidebar-wrapper').toggleClass("close_icon");
      });
      };
...
if($('.simplebar-wrapper .simplebar-content-wrapper') && $('#pageWrapper').hasClass('compact-wrapper')) {
  $('.simplebar-wrapper .simplebar-content-wrapper').animate({
      scrollTop: $('.simplebar-wrapper .simplebar-content-wrapper a.active').offset() - 400
  }, 1000);
}
