...
<!--</button>-->
    ...
    <div class="rop_regseter">
      ...
      <label for="">
        By creating an account, you agree to our <a href="https://milliondox.com/legal.html">Terms & Conditions</a>
      </label>
      </div>
      ...
      <style>
          .submit_forrm:disabled {
    cursor: not-allowed;
    background-color: #ccc; /* Optional: change background to indicate it's disabled */
...
<script>
  $(document).ready(function() {
    // Initially disable the submit button and apply the disabled styling
    $('.submit_forrm').prop('disabled', true);

    // Function to check if all required fields and the checkbox are filled/checked
    function checkFormFields() {
        let allFilled = true;

        // Iterate over each required field in the form
        $('#registerForm input[required]').each(function() {
            // Check if the input is a checkbox
            if ($(this).attr('type') === 'checkbox') {
                if (!$(this).is(':checked')) {
                    allFilled = false; // If the checkbox is not checked, set allFilled to false
                }
            } else if ($(this).val() === '') {
                allFilled = false; // If any other required field is empty, set allFilled to false
            }
        });

        // Enable or disable the register button based on whether all required fields are filled
        if (allFilled) {
            $('.submit_forrm').prop('disabled', false);
        } else {
            $('.submit_forrm').prop('disabled', true);
        }
    }

    // Trigger the check whenever the user types or changes any input in the form
    $('#registerForm input').on('input change', function() {
        checkFormFields();
    });

    // Also check when the page is loaded (in case some fields are pre-filled)
    checkFormFields();
});
