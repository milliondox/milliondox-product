...
.progress-wrapper {
    width: 100%; /* Full width of the minimized modal */
    background-color: #e0e0e0; /* Light gray background for progress */
...
.tick-icon {
    ...
    margin-left: 10px; /* Space between progress bar and tick */
    color: green; /* Color of the tick */
    font-size: 24px; /* Size of the tick icon */
    align-self: center; /* Center vertically */
}

.minimizedModal:hover {
    box-shadow: 0 4px 15px rgba(0, 0, 0, 0.3); /* Slightly increase shadow on hover */
}
...
$(document).ready(function () {
    ...
    $('body').on('change', '.dragfile', function () {
        ...
        var fileList = fileArea.next('.file-list');

        handleFiles(fileInput, fileList);
    });
    ...
    $('.file-area').on('dragleave drop', function (event) {
        event.preventDefault();
        $(this).removeClass('green-outline');
    });

    $('.file-area').on('drop', function (event) {
        event.preventDefault();
        var fileInput = $(this).find('.dragfile')[0];
        var fileList = $(this).next('.file-list');
        fileInput.files = event.originalEvent.dataTransfer.files;
        handleFiles(fileInput, fileList);
    });

    // Function to handle files
    function handleFiles(fileInput, fileList) {
        ...
        files.forEach((file, index) => {
            const fileItem = $('<div>', { class: 'file-item' }).append(
                ...
                $('<button>', { class: 'remove-btn', 'data-file-index': index }).text('X').on('click', function () {
                    removeFile(fileInput, $(this).data('file-index'), fileList);
                })
            );
    ...
    function removeFile(fileInput, index, fileList) {
        var files = Array.from(fileInput.files);
