...
<div class="page-header">
    <div class="header-wrapper row m-0">
      ...
      <div class="container-fluid  employee-lifecycle em-details">
        <div class="row">
          <div class="col-md-12">
...
<div class="hearder-backk">
                    <a href="{{url('/user/Employeelifecycle')}}"><svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<div class="role_name">
    ...
    <span>{{$emp->position}}</span>
        ...
        <td>
            <span class="s_date">Start date</span>
            <span class="s_date_date">{{$emp->startdate}}</span>
        </td>
        <td>
            <span class="s_date">Exit date</span>
            @if($startend <= $today)
            <span class="emp_stst">{{$emp->startend}}</span>
                @else
                <span class="s_date_date">-</span>
                    
                @endif
            
        </td>
        <td>
            <a class="go_view" data-bs-toggle="modal" data-bs-target="#Edit_emp">Edit </a>
            <!-- model start -->
<div class="modal fade drop_coman_file have_title" id="Edit_emp" tabindex="-1" role="dialog" aria-labelledby="Edit_emp" aria-hidden="true">
                              <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <h5 class="modal-title" style="font-weight:700">Edit Employee</h5>
                                  ...
                                  <div class="modal-body">
                                    ...
                                    <form action="{{ route('upstoreemployeeprofile') }}" method="POST" enctype="multipart/form-data" class="upload-form"> 
                                      @csrf
                          ...
                          <div class="inner_file_edit">	    
                                    <input type="file" id="emppic" name="file">   
                                    <div class="dont_show"> 
                                    <img src="{{  asset('/' . $emp->file_path)  }}" alt="img">
                          ...
                          <input placeholder="Full Name" type="text" id="fname" name="fname" value="{{$emp->fname}}">
                          <input  type="hidden" id="empid" name="empid" value="{{$emp->id}}">
                          </div>

                          <div class="gropu_form">
                          <label for="fname">Position*</label>
                          <input placeholder="Job Position" type="text" id="position" value="{{$emp->position}}" name="position">
                          </div>
  ...
  <option value="" disabled Selected>select</option>
    <option value="Human Resources" {{ $emp->division === 'Human Resources' ? ' selected' : '' }}>Human Resources</option>
    <option value="Admin" {{ $emp->division === 'Admin' ? ' selected' : '' }}>Admin</option>
    <option value="Sales" {{ $emp->division === 'Sales' ? ' selected' : '' }}>Sales</option>
    <option value="Development" {{ $emp->division === 'Development' ? ' selected' : '' }}>Development</option>
    <option value="Security" {{ $emp->division === 'Security' ? ' selected' : '' }}>Security</option>
    <option value="Financial accounting" {{ $emp->division === 'Financial accounting' ? ' selected' : '' }}>Financial accounting</option>
    <option value="IT" {{ $emp->division === 'IT' ? ' selected' : '' }}>IT</option>
    <option value="Business Development" {{ $emp->division === 'Business Development' ? ' selected' : '' }}>Business Development</option>
    <option value="Marketing" {{ $emp->division === 'Marketing' ? ' selected' : '' }}>Marketing</option>
    <option value="Purchasing" {{ $emp->division === 'Purchasing' ? ' selected' : '' }}>Purchasing</option>
    <option value="Research and development" {{ $emp->division === 'Research and development' ? ' selected' : '' }}>Research and development</option>
    <option value="Finance" {{ $emp->division === 'Finance' ? ' selected' : '' }}>Finance</option>
    <option value="Accounts" {{ $emp->division === 'Accounts' ? ' selected' : '' }}>Accounts</option>
    <option value="Asset management" {{ $emp->division === 'Asset management' ? ' selected' : '' }}>Asset management</option>
    <option value="Communication" {{ $emp->division === 'Communication' ? ' selected' : '' }}>Communication</option>
    <option value="Human resource development" {{ $emp->division === 'Human resource development' ? ' selected' : '' }}>Human resource development</option>
    <option value="Quality" {{ $emp->division === 'Quality' ? ' selected' : '' }}>Quality</option>
    <option value="Technical Support" {{ $emp->division === 'Technical Support' ? ' selected' : '' }}>Technical Support</option>
    <option value="CTO" {{ $emp->division === 'CTO' ? ' selected' : '' }}>CTO</option>
    <option value="Customer service" {{ $emp->division === 'Customer service' ? ' selected' : '' }}>Customer service</option>
    <option value="Database administration" {{ $emp->division === 'Database administration' ? ' selected' : '' }}>Database administration</option>
    <option value="Design" {{ $emp->division === 'Design' ? ' selected' : '' }}>Design</option>
    <option value="Dispatch department" {{ $emp->division === 'Dispatch department' ? ' selected' : '' }}>Dispatch department</option>
    <option value="Helpdesk" {{ $emp->division === 'Helpdesk' ? ' selected' : '' }}>Helpdesk</option>
...
<input type="date" id="start" name="startdate" value="{{$emp->startdate}}"  />
                          </div>

                          
                          <div class="gropu_form">
                          <label for="start">Exit Date</label>
<input type="date" id="start" name="startend" value="{{$emp->startend}}"  />
                          </div>
...
<!-- model end -->
                    ...
                    <div class="hearder-entres">
                      ...
                      <div class="sadd_filds">
                        <button class="btn hvr-rotate" id="add_lifecycle"> + Add </button>                        
                      </div>                      
...
<td>
                            <div class="dropdown" >
                              ...
                              <div id="myDropdown" class="dropdown-content"> <div class="down_del">
                                  <a class=" dropdown-itemm dnlod_nt" href="{{ route('download-empdt-file', ['id' => $empt->detail_id]) }}">
                                  ...
                                  <form method="POST" action="{{ route('fileempdt.delete', ['id' => $empt->detail_id]) }}">
                                            @csrf
                                            @method('DELETE')
...
<rect width="4.27086" height="66.3713" transform="matrix(-0.704896 -0.70931 0.706518 -0.707695 3.10742 50)" fill="black"/>
                                            ...
                                            <h3>Edit Doc</h3>
                                            <form action="{{ route('updateemployeedetail') }}" method="POST" enctype="multipart/form-data" class="upload-form"> 
                                      @csrf
...
<a class=" dropdown-itemm dnlod_nt" href="{{ route('download-empdt-file', ['id' => $empt->detail_id]) }}" style="margin-top:10px; display: block;">
...
<td>
  @foreach($emplife as $emp)
<form action="{{ route('storeemployeedetail') }}" method="POST" enctype="multipart/form-data" class="upload-form">
 @csrf
...
@endforeach
    ...
    document.getElementById('add_lifecycle').addEventListener('click', function() {
