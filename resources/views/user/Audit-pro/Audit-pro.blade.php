@extends('user.includes.Audit-pro') @section('content')
    <!-- tap on top starts-->
...
<div class="page-header">
    ...
    <tr class="active" >
                          ...
                          <td>{{$audi->file_name}} </td>
                          <td>
                          {{$audi->file_type}}                         
                          </td>
                         
                          @php
                $fileSizeKB = round($audi->file_size / 1024, 2) ; // Convert bytes to KB and round to 2 decimal places
            @endphp
                            <td>{{$fileSizeKB }} KB </td>
...
<td>
                            <div class="dropdown" >
                              ...
                              <div id="myDropdown" class="dropdown-content"> <div class="down_del">
                                  <a class=" dropdown-itemm dnlod_nt" href="{{ route('download-audit-file', ['id' => $audi->id]) }}">
                                  ...
                                  <a class="dropdown-itemm delet_nt" id="delete-{{ $audi->id }}">
                                  <form method="POST" action="{{ route('fileaudit.delete', ['id' => $audi->id]) }}">
                                                            @csrf
                                                            @method('DELETE')
                                ...
                                <a class="dropdown-itemm rename_nt" data-bs-toggle="modal" data-bs-target="#edit_histry_1{{ $audi->id }}">
                                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M8.75 9.33323L6.41667 11.6666H12.25V9.33323H8.75ZM7.035 4.19406L1.75 9.47906V11.6666H3.9375L9.2225 6.38156L7.035 4.19406ZM10.9142 4.6899C11.1417 4.4624 11.1417 4.08323 10.9142 3.8674L9.54917 2.5024C9.4398 2.39389 9.29198 2.33301 9.13792 2.33301C8.98385 2.33301 8.83604 2.39389 8.72667 2.5024L7.65917 3.5699L9.84667 5.7574L10.9142 4.6899Z" fill="#8D8D8D" />
                                           ...
                                           <!-- model start -->
<div class="modal fade" id="edit_histry_1{{ $audi->id }}" tabindex="-1" role="dialog" aria-labelledby="edit_histry_1{{ $audi->id }}" aria-hidden="true">
...
<rect width="4.27086" height="66.3713" transform="matrix(-0.704896 -0.70931 0.706518 -0.707695 3.10742 50)" fill="black"/>
                                            ...
                                            <h3>Edit Doc</h3>
                                            <form action="{{ route('upstoreaudit') }}" method="POST" enctype="multipart/form-data" class="upload-form"> 
                                      @csrf
                                      <input type="hidden" name="record_id1" value="{{ $audi->id }}">
                                              <div class="form-grouup">
<label for="fname">File  name:</label>
<input type="text" id="file_name" name="file_name" value="{{ $audi->file_name }}">
</div>
<div class="form-grouup">
<label for="lname">Doc File:</label>
<input type="file" id="auditfile" name="auditfile" accept=".pdf,.doc,.docx" >
...
<td>
<form action="{{ route('storeaudit') }}" method="POST" enctype="multipart/form-data" class="upload-form">
 @csrf
...
<input type="file" id="auditfile"  name="auditfile" required>
